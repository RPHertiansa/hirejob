          ...
          <div class="col-6 login-right">
              ...
              <p class="mb-5" style="font-size: 18px; line-height: 25px;color: #46505C;">Lorem ipsum dolor sit amet, consectetur adipiscing elit. In euismod ipsum et dui rhoncus auctor.</p>
               <form @submit.prevent="login">
                ...
                <router-link style="font-size: 16px;line-height: 22px;color: #1F2A36;" to="/forgotpass-perekrut" class="mb-3 float-right">Lupa kata sandi?</router-link>
                <button type="submit" class="btn btn-primary btn-lg btn-block">Masuk</button>
              </form>
              <p class="mt-4 text-center">Anda belum punya akun?<router-link to="/register-perekrut" class="ml-1" style="color:#FBB017;">Daftar disini</router-link></p>
          </div>
      </div>
  </div>
  <!-- Responsive -->
...
export default {
  ...
  methods: {
    ...mapActions({
      onLogin: 'auth/onLoginPerekrut'
    }),
    login () {
      const data = {
        emailperekrut: this.loginEmail,
        passwordperekrut: this.loginPassword
      }
      this.onLogin(data).then(result => {
        if (result === "Cannot read property 'passwordperekrut' of undefined") {
          this.alertExist()
